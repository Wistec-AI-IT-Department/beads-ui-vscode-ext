...
import { getGitHubConfigForWorkspace, syncGitHubToBeads, checkGhAuth, setActiveWorkspace } from './github-sync.js';
...
export async function handleMessage(ws, data) {
  ...
  if (req.type === 'list-workspaces') {
    ...
    // Enrich workspaces with GitHub repo info
    const workspaces = rawWorkspaces.map(ws => {
      const ghConfig = getGitHubConfigForWorkspace(ws.path);
      return {
        ...ws,
        github: ghConfig ? `${ghConfig.owner}/${ghConfig.repo}` : null
      };
    });
  ...
  if (req.type === 'set-workspace') {
    ...
    if (changed) {
      ...
      // Update active workspace for GitHub sync
      ...
      // Trigger GitHub sync for the new workspace
      (async () => {
        try {
          const auth = await checkGhAuth();
          if (auth.authenticated) {
            const result = await syncGitHubToBeads(resolved);
            if (result.ok) {
              log('github sync after workspace switch: %d created', result.results?.created || 0);
              scheduleListRefresh();
            }
          }
        } catch (err) {
          log('github sync after workspace switch failed: %o', err);
        }
      })();
    }
