...
/**
 * Get the display name for a workspace. Prefers GitHub repo name if available,
...
function getDisplayName(workspace) {
  // If GitHub repo info is available, use it
  if (workspace.github) {
    return workspace.github;
  }
  // Fall back to folder name
  return getProjectName(workspace.path);
}
...
export function createWorkspacePicker(mount_element, store, onWorkspaceChange) {
  ...
  function template() {
    ...
    if (available.length === 1) {
      const name = getDisplayName(available[0]);
      return html`
