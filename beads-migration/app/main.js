...
import { createTelemetryView } from './views/telemetry.js';
...
export function bootstrap(root_element) {
  ...
  const shell = html`
    ...
    <section id="telemetry-root" class="route telemetry" hidden></section>
  ...
  /** @type {HTMLElement|null} */
  const telemetry_root = document.getElementById('telemetry-root');

  /** @type {HTMLElement|null} */
  const list_mount = document.getElementById('list-panel');
  /** @type {HTMLElement|null} */
  const detail_mount = document.getElementById('detail-panel');
  if (list_mount && issues_root && epics_root && board_root && telemetry_root && detail_mount) {
    /** @type {HTMLElement|null} */
    ...
    const telemetry_view = createTelemetryView(telemetry_root, store, client);
    // Preload epics when switching to view
    /**
     * @param {{ selected_id: string | null, view: 'issues'|'epics'|'board'|'telemetry', filters: any }} s
     */
    ...
    const onRouteChange = (s) => {
      if (issues_root && epics_root && board_root && telemetry_root && detail_mount) {
        ...
        telemetry_root.hidden = s.view !== 'telemetry';
      ...
      if (!s.selected_id && s.view === 'telemetry') {
        void telemetry_view.load();
      }
